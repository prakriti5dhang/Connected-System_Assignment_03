...
public class ExampleAgent extends SCMAgent {
    ...
    protected void handleCustomerRFQs(RFQBundle rfqBundle) {
        BOMBundle bom = getBOMBundle();

        for (int i = 0; i < rfqBundle.size(); ++i) {
            int dueDate = rfqBundle.getDueDate(i);
            if (dueDate - getCurrentDate() < 6 || dueDate > this.lastBidDueDate)
                continue;

            int reservePricePerUnit = rfqBundle.getReservePricePerUnit(i);
            int productIndex = bom.getIndexFor(rfqBundle.getProductID(i));
            int productBasePrice = productIndex >= 0 ? bom.getProductBasePrice(productIndex) : 0;
            productBasePrice *= 0.9;
            if (reservePricePerUnit <= productBasePrice || this.random.nextDouble() >= this.bidProbability)
                continue;

            //Place yourself on the scale between max price and base price, randomly.
            int unitPriceOffer = (int)(productBasePrice + (reservePricePerUnit - productBasePrice)
                    * (1.0 - this.random.nextDouble() * this.priceDiscountFactor));
            addCustomerOffer(rfqBundle, i, unitPriceOffer);
        }
        sendCustomerOffers();
    }

    protected void handleFactoryStatus(FactoryStatus status) {
        double d = status.getUtilization();
        if (d > 0.9) {
            this.bidProbability -= 0.1;
            if (this.bidProbability < 0.0) {
                this.bidProbability = 0.0;
            }
        } else if (d < 0.4) {
            this.bidProbability += 0.05;
            if (this.bidProbability > 1.0) {
                this.bidProbability = 1.0;
            }
        }
    }
